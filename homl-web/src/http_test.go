...
import (
	...
	"github.com/alkariin/homl/homl-web/service/mocks"
...
func newTestServer() (*gin.Engine, *serverMocks) {
	...
	handler := &controller.Handler{
		UsersService:      sm.users,
		CategoriesService: sm.categories,
		EventsService:     sm.events,
		TagsService:       sm.tags,
		PersonsService:    sm.persons,
		SettingsService:   sm.settings,
	}
	router := SetupRouter(handler, "", 5*time.Second)
	return router, sm
}

// authHeader returns a Bearer token accepted by TokenAuthMiddleware for testUserID.
func authHeader() string {
	td, _ := helper.CreateToken(testUserID)
...
func TestRegistrationEndpoint(t *testing.T) {
	...
	sm.users.On("Registration", mock.AnythingOfType("*model.User"), mock.AnythingOfType("*model.Language")).
		Return(map[string]string{"access_token": "a", "refresh_token": "r"}, nil)

	rec := doRequest(router, http.MethodPost, "/registration",
		`{"username":"someone@example.com","password":"masterdev","language":"fr"}`, "")
...
func TestRegistrationRejectsInvalidLanguage(t *testing.T) {
	...
	rec := doRequest(router, http.MethodPost, "/registration",
		`{"username":"someone@example.com","password":"masterdev","language":"es"}`, "")
...
func TestLoginEndpoint(t *testing.T) {
	...
	sm.users.On("Login", mock.AnythingOfType("*model.User")).
		Return(map[string]string{"access_token": "a", "refresh_token": "r"}, nil)

	rec := doRequest(router, http.MethodPost, "/login",
		`{"username":"someone@example.com","password":"masterdev"}`, "")
...
func TestRefreshEndpoint(t *testing.T) {
	...
	sm.users.On("Refresh", mock.AnythingOfType("*model.RefreshInput")).
		Return(map[string]string{"access_token": "a", "refresh_token": "r"}, nil)
...
func TestLogoutEndpoint(t *testing.T) {
	...
	sm.users.On("Logout", mock.AnythingOfType("*model.AccessDetails")).Return(nil)
...
func TestGetCategoriesEndpoint(t *testing.T) {
	...
	sm.categories.On("GetCategories", testUserID).Return([]model.GetCategoryResponse{
	...
	assert.Equal(t, http.StatusOK, rec.Code)
	var out []model.GetCategoryResponse
...
func TestCreateCategoryEndpoint(t *testing.T) {
	...
	sm.users.On("GetUserIdFromToken", mock.Anything).Return(testUserID, nil)
	sm.categories.On("CreateCategory", mock.AnythingOfType("*model.Category")).Return(nil)
...
func TestUpdateCategoryEndpoint(t *testing.T) {
	...
	sm.users.On("GetUserIdFromToken", mock.Anything).Return(testUserID, nil)
	sm.categories.On("UpdateCategory", mock.AnythingOfType("*model.Category")).Return(nil)
...
func TestGetEventsEndpoint(t *testing.T) {
	...
	sm.users.On("GetUserIdFromToken", mock.Anything).Return(testUserID, nil)
	sm.events.On("GetEvents", testUserID, mock.Anything).Return([]model.GetEventsResponse{
		{Event: model.Event{Id: 1, Description: "cool"}},
	}, nil)

	rec := doRequest(router, http.MethodGet, "/events", `{"tags":[]}`, authHeader())

	assert.Equal(t, http.StatusOK, rec.Code)
	var out []model.GetEventsResponse
	assert.NoError(t, json.Unmarshal(rec.Body.Bytes(), &out))
	assert.Len(t, out, 1)
...
func TestCreateEventEndpoint(t *testing.T) {
	...
	sm.users.On("GetUserIdFromToken", mock.Anything).Return(testUserID, nil)
	sm.events.On("CreateEvent", testUserID, mock.AnythingOfType("*model.Event"), mock.Anything).Return(nil)
