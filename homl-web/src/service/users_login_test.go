...
func TestLogin(t *testing.T) {
	...
	t.Run("Returns an access/refresh token pair on valid credentials", func(t *testing.T) {
		...
		stored := &model.User{ID: 1, Username: "demo@homl.local", Password: string(hashed)}
		mockRepo.On("FindByUsername", "demo@homl.local").Return(stored, nil)
		mockRepo.On("CreateAuth", uint64(1), mock.AnythingOfType("*model.TokenDetails")).Return(nil)

		tokens, err := svc.Login(&model.User{Username: "demo@homl.local", Password: password})
	...
	t.Run("Rejects a wrong password without issuing tokens", func(t *testing.T) {
		...
		tokens, err := svc.Login(&model.User{Username: "demo@homl.local", Password: "wrong-password"})
	...
	t.Run("Rejects an unknown user", func(t *testing.T) {
		...
		tokens, err := svc.Login(&model.User{Username: "ghost@homl.local", Password: password})
...
func TestLogout(t *testing.T) {
	t.Run("Clears the challenge and deletes the auth entry", func(t *testing.T) {
		...
		ad := &model.AccessDetails{AccessUuid: "uuid-1", UserId: 1}
	...
	t.Run("Errors when no auth entry was deleted", func(t *testing.T) {
		...
		ad := &model.AccessDetails{AccessUuid: "uuid-missing", UserId: 1}
		mockRepo.On("UpdateChallenge", uint64(1), (*string)(nil)).Return(nil)
