...
func TestCreateCategory(t *testing.T) {
	t.Run("Encrypts and title-cases the name, then forwards to the repository", func(t *testing.T) {
		...
		mockRepo.On("Create", mock.MatchedBy(func(c *model.Category) bool {
			dec, err := helper.Decrypt(c.Category)
			return err == nil && dec == "Noces" && c.Color == "red" && !c.IsLocked && c.IdUser == 42
		})).Return(nil)

		err := svc.CreateCategory(&model.Category{Category: "noces", Color: "red", IdUser: 42})
	...
	t.Run("Propagates repository errors", func(t *testing.T) {
		...
		err := svc.CreateCategory(&model.Category{Category: "Noces", Color: "red"})
...
func TestUpdateCategory(t *testing.T) {
	t.Run("Forbids renaming a locked category", func(t *testing.T) {
		...
		oldEncrypted, _ := helper.Encrypt("Dates")
		mockRepo.On("FindById", uint(1)).Return(&model.Category{
			Id:       1,
			Category: oldEncrypted,
			IsLocked: true,
		}, nil)

		err := svc.UpdateCategory(&model.Category{Id: 1, Category: "Renamed", Color: "#ffffff"})
	...
	t.Run("Updates a non-locked category", func(t *testing.T) {
		...
		stored, _ := helper.Encrypt("Holidays")
		mockRepo.On("FindById", uint(2)).Return(&model.Category{
			Id:       2,
			Category: stored,
			IsLocked: false,
		}, nil)
		mockRepo.On("Update", mock.MatchedBy(func(c *model.Category) bool {
			dec, err := helper.Decrypt(c.Category)
			return err == nil && dec == "Trips" && c.Color == "#000000"
		})).Return(nil)

		err := svc.UpdateCategory(&model.Category{Id: 2, Category: "Trips", Color: "#000000"})
...
func TestGetCategories(t *testing.T) {
	t.Run("Decrypts names and returns them sorted by id", func(t *testing.T) {
		...
		encA, _ := helper.Encrypt("Dates")
		encB, _ := helper.Encrypt("Persons")
