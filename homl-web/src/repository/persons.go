...
func NewPersonsRepository(db *sql.DB) model.PersonsRepository {
	return &PersonsRepository{
		DB: db,
	}
}

func (r *PersonsRepository) FindById(idPerson uint) (*model.Person, error) {
...
func (r *PersonsRepository) FindPersonsWithTagsAndCategories(idUser uint64) (map[uint]model.Person, map[uint][]model.Nickname, error) {
	...
	nicknames := make(map[uint][]model.Nickname)
	...
	for results.Next() {
		...
		decNickname, err := helper.Decrypt(nickname.Nickname)
		if err != nil {
			return nil, nil, err
		}
		nickname = model.Nickname{Id: nickname.Id, Nickname: decNickname}
...
func (r *PersonsRepository) CreatePersonWithTags(encFirstname string, encLastname string, encMainTagName string, idCategoryPerson uint, nicknames []string) error {
	...
	for _, nickname := range nicknames {
		encNickname, err := helper.Encrypt(nickname)
		if err != nil {
			tx.Rollback()
			return err
		}
	...
	for i := 1; i < len(nicknames); i++ { // i=1 to skip the main tag
		...
		if lastIndex >= len(bodyNicknames) {
			...
			for j := lastIndex; j < len(bodyNicknames); j++ {
				...
				if n2.Id == 0 {
					encNickname, err := helper.Encrypt(n2.Nickname)
					if err != nil {
						tx.Rollback()
						return err
					}
