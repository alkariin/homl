...
func NewTagsRepository(db *sql.DB) model.TagsRepository {
...
func CreateAllTags(ctx context.Context, tx *sql.Tx, tags []model.Tag) ([]uint, error) {
	// Create date tags if needed
	var tagsId = []uint{}
	for _, tag := range tags {
		if tag.Id == 0 {
			encTag, err := helper.Encrypt(tag.Tag)
			if err != nil {
				tx.Rollback()
				return nil, err
			}
