...
func NewEventsRepository(db *sql.DB) domain.EventsRepository {
	return &EventsRepository{
		DB: db,
	}
}

func (e *EventsRepository) FindEventsWithTags(encTags []string, idUser uint64) (map[uint]domain.Event, map[uint][]domain.Tag, error) {
	...
	resTags := make(map[uint][]domain.Tag)
	resEvents := make(map[uint]domain.Event)
	for results.Next() {
		var tag domain.Tag
		...
		decTag, err := shared.Decrypt(tag.Tag)
		if err != nil {
			return nil, nil, err
		}
...
func (e *EventsRepository) CreateEventWithTags(tags []domain.Tag, tagsId []uint, event *domain.Event, idUser uint64) error {
	...
	otherTagsId, err := tag.CreateAllTags(ctx, tx, tags)
	if err != nil {
		return err
	}
	...
	encDescription, err := shared.Encrypt(event.Description)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func (e *EventsRepository) UpdateEventWithTags(tags []domain.Tag, tagsId []uint, event *domain.Event, idUser uint64) error {
	ctx := context.Background()
	tx, err := e.DB.BeginTx(ctx, nil)
	if err != nil {
		return err
