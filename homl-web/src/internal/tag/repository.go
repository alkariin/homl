...
func NewTagsRepository(db *sql.DB) domain.TagsRepository {
...
func CreateAllTags(ctx context.Context, tx *sql.Tx, tags []domain.Tag) ([]uint, error) {
	// Create date tags if needed
	var tagsId = []uint{}
	for _, tag := range tags {
		if tag.Id == 0 {
			encTag, err := shared.Encrypt(tag.Tag)
			if err != nil {
				tx.Rollback()
				return nil, err
			}
